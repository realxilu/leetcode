class Solution:
    def findItinerary(self, tickets: List[List[str]]) -> List[str]:
        ...
        while stack:
            peek_item = stack[-1]
            if peek_item in graph and len(graph[peek_item]) > 0:
                # Check if peek_item in graph as there may be a case when there is no out edge from an airport
                # In that case it won't be present as a key in graph
                stack.append(graph[peek_item].pop())
            else:
